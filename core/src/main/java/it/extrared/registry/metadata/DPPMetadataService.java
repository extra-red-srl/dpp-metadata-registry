...
public class DPPMetadataService {
    ...
    /**
     * Save or update a metadata entry by executing the autocompletion if provided. The way in which data should be updated
     * depends upon the configured {@link MetadataRegistryConfig#updateStrategy()} i.e. property registry.update-strategy.
     ...
     * @return the saved/updated {@link DPPMetadata}.
     */
    public Uni<DPPMetadata> saveOrUpdate(JsonNode metadata, List<String> autocompleteBy) {
        return pool.withTransaction(c -> saveOrUpdateInternal(c, metadata, autocompleteBy));
    }

    private Uni<DPPMetadata> saveOrUpdateInternal(
        ...
        DPPMetadata incoming = new DPPMetadata(metadata);
    ...
    private Uni<? extends DPPMetadata> doUpdate(
            DPPMetadata modifier, DPPMetadata modified, SqlConnection conn) {
    ...
    private Uni<? extends DPPMetadata> doSave(DPPMetadata incoming, SqlConnection connection) {
    ...
    private Uni<Void> applyAutoComplete(
        ...
        if (autocompleteBy != null
            ...
            Uni<DPPMetadata> dppMetadata = repository.findBy(con, filters);
            return dppMetadata
                    .invoke(
                            m -> {
                                if (m != null)
    ...
    private Uni<Void> validate(DPPMetadata metadata) {
        return schemaCache
                .get()
                .invoke(
                        s -> {
