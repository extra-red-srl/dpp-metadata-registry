...
/**
 * Base interface for a JsonSchemaLoader, able to read a DPP metadata json schema document for a given source.
 */
public interface JsonSchemaLoader {
    ...
    /**
     * Return the priority of a schema loader over the others (the lower the number the greater the priority).
     * @return the priority of a schema loader as an {@link java.lang.Integer}.
     */
    Integer priority();

    /**
     * Set the next schema loader to be executed after this one in case this loader is not able to retrieve the schema.
